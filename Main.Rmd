...
|:--|:--|:--|:--|
| Función de densidad versus curva normal| Anote lo observado| Anote lo observado| Anote lo observado|
| Conclusión| | | |
| QQ-Plot| Anote lo observado| Anote lo observado| Anote lo observado|
| Conclusión| | | |
| S-W test| Valor P| Valor P| Valor P|
| Conclusión| | | |
| A-D test| Valor P| Valor P| Valor P|
| Conclusión| | | |
| D'Agostino-Pearson| Valor P| Valor P| Valor P|
| Conclusión| | | |
| Curtosis y simetría| Valor P| Valor P| Valor P|
| Conclusión| | | |
| Conclusión general sobre normalidad de los datos| | | |
...
| Dato| Valor|
|:--|:--|
| Tipo de prueba a utilizar| Valor|
| Valor observado| Valor|
| Grados de libertad y qué representan| Valor|
| Valor P| Valor|
| $H_{0}$| Valor|
| $H_{1}$| Valor|
...
En caso de detectar alguna diferencia en cuanto a la seguridad que puede brindar al tamaño del vehículo, ¿es posible en cuáles existe una verdadera diferencia significativa?
...
## Caso 6:
Primero se carga los datos del documento DatosVentas.csv
```{r}
...
#1
Se crea un modelo con la variable  y = ventas y x = tv 
```{r}
mod0 <- lm(ventas ~ tv, data = ventas)
mod0
```
...
#2
Se crea un modelo con la variable  y = ventas, x1 = tv, x2 = radio y x3 = periodico
```{r}
...
Tras optener los coeficientes y realizar el summery al modelo podemos concluir que los que aportan o son significantes para a modelo son los de la variable tv, radio y el intercepto, mientras que la varible de periodico no es significativa para el modelo.

#3
Se crea un nueco modelo con las variables significativas variable  y = ventas y x1 = tv y x2 = radio
```{r}
...
En este caso tras realizar el summary de el modelo, podemos ver todas las variables son significativas para el modelo.
...
Tras optener los resultados y realizar el summary podemos ver que todos los coeficientes obtenidos son significativos y aportan al modelo.
...
#1
A continuación se muestra una grafica con los datos resumidos de los coeficiente de correlación de todos los modelos asi como coeficiente de determinación.
...
plot(lm0,lm2, lm3)
```
Como podemos ver en el gráfico el mejor modelo el el mod3 ya que presenta la curva mas baja de la grafica, como segundo mejor modelo el mod2 y como peor modelo el mod1.
...
```
Como podemos ver tras realizar la prueba de anova el resultado nos muestra nuevamente que el modelo mod3 es el mejor para utilizar en nuestras predicciones.
