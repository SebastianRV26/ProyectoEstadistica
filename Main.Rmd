...
De los resultados obtenidos se puede concluir que la media de precios del dataset diamante se encuentra en el intervalo ]`r IC`[, resultado que coincide en las tres librerías utilizadas.
...
De los resultados obtenidos, se puede concluir que la media de precios del dataset de diamantes se encuentra en el intervalo ]`r IC`[. Además de esto, comparándolo con el IC de la distribución Z se puede observar que este posee menor precisión debido a la poca cantidad de datos que fueron brindados.
...
Dando como resultado el IC para la varianza de quilates:
...
\(H_{0}\) no puede ser rechazado dado que no se encontró evidencia suficiente en contra, ya que según lo observado el 0.12 se encuentra dentro del intervalo de aceptación y el valor p es mayor que el valor de alfa, por lo que lo ubica en la región de aceptación, y por ende se asume como verdadera la afirmación del joyero.  
